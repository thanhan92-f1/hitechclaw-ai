...
import { sessionsCollection, messagesCollection, llmLogsCollection, handoffSessionsCollection, } from '@hitechclaw/db';
    ...
    app.get('/overview', async (c) => {
        try {
            ...
            const [totalConversations, totalMessages, recentSessions] = await Promise.all([
                ...
                messages.countDocuments({ tenantId: tenantId, createdAt: { $gte: since } }),
        ...
        try {
            ...
            for (const session of recentSessions) {
                ...
                sentimentResults.push({
                    ...
                    title: session.title || session._id,
                    sentiment,
    ...
    app.get('/topics', async (c) => {
        try {
            ...
            const userMessages = await msgsColl
                .find({ tenantId: tenantId, role: 'user', createdAt: { $gte: since } })
            ...
            for (const msg of userMessages) {
                ...
                for (const topic of topics) {
                    ...
                    if (entry.samples.length < 3)
                        entry.samples.push(content.slice(0, 120));
