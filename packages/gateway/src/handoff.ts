...
export function createHandoffRoutes() {
  ...
  app.put('/rules/:ruleId', async (c) => {
    try {
      ...
      const updates = await c.req.json<Partial<{ enabled: boolean; config: Record<string, unknown> }>>();

      const { ObjectId: ObjId } = await import('mongodb');
      await escalationRulesCollection().updateOne(
        { _id: new ObjId(ruleId), tenantId },
  ...
  app.get('/queue', async (c) => {
    try {
      const tenantId = c.get('tenantId') as string;
      const status = (c.req.query('status') || 'pending') as 'pending' | 'assigned' | 'active' | 'resolved' | 'returned_to_ai';
  ...
  app.post('/:handoffId/assign', async (c) => {
    try {
      ...
      const { ObjectId: ObjId } = await import('mongodb');
      const result = await handoffSessionsCollection().findOneAndUpdate(
        { _id: new ObjId(handoffId), tenantId, status: 'pending' },
        { $set: { status: 'assigned', agentUserId, assignedAt: new Date() } },
  ...
  app.post('/:handoffId/resolve', async (c) => {
    try {
      const tenantId = c.get('tenantId') as string;
      const handoffId = c.req.param('handoffId');
      const { returnToAI } = await c.req.json<{ returnToAI?: boolean }>().catch(() => ({ returnToAI: false }));

      const { ObjectId: ObjId } = await import('mongodb');
      const newStatus = returnToAI ? 'returned_to_ai' : 'resolved';

      const result = await handoffSessionsCollection().findOneAndUpdate(
        { _id: new ObjId(handoffId), tenantId },
  ...
  app.get('/stats', async (c) => {
    try {
      ...
      if (resolvedRecent.length > 0) {
        const totalMs = resolvedRecent.reduce((sum: number, h: { resolvedAt?: Date; assignedAt?: Date }) => {
...
export async function checkEscalationTriggers(
  ...
  sessionId: string,
  userId: string,
): Promise<{ shouldEscalate: boolean; reason?: string; detail?: string }> {
  try {
    const rules = await escalationRulesCollection()
