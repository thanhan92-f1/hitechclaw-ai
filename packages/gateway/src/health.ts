...
export function createHealthRoutes() {
  ...
  app.get('/health/deep', async (c) => {
    const checks: Record<string, { status: string; latency_ms: number; error?: string; details?: Record<string, unknown> }> = {};
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const modelCount = llmProvider === 'ollama' && Array.isArray((data as any).models)
      ...
      if (overall === 'ok') overall = 'degraded'; // LLM down → degraded (not unhealthy, app can still serve)
    }
