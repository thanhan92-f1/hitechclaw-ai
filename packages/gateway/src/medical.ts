...
const CRITICAL_INTERACTIONS: Array<{
  drugs: [string, string];
...
function getDrugClass(drug: string): string {
  const lower = drug.toLowerCase();
  for (const [cls, members] of Object.entries(DRUG_CLASSES)) {
    if (members.some((m) => lower.includes(m)) || lower.includes(cls)) return cls;
  }
  return lower;
}
...
const MEDICAL_TEMPLATES: Array<{
  id: string;
  name: string;
  category: string;
  description: string;
  fields: Array<{ name: string; label: string; type: 'text' | 'textarea' | 'select' | 'date'; options?: string[]; required?: boolean }>;
  template: string;
}> = [
  {
    id: 'soap-note',
...
export function createMedicalRoutes(ctx: GatewayContext) {
  const app = new Hono();
  ...
  app.post('/icd10', async (c) => {
    const body = await c.req.json() as { query?: string; category?: string };
    ...
    if (body.category) {
      results = results.filter((code) =>
        code.category.toLowerCase() === body.category!.toLowerCase(),
      );
    }
...
Format as a proper SOAP note with Subjective, Objective, Assessment, and Plan sections. Include appropriate ICD-10 codes in the Assessment. Be thorough but concise.`;
  ...
  app.post('/clinical-alert', async (c) => {
    ...
    // Sort by severity
    const severityOrder: Record<string, number> = { emergency: 0, urgent: 1, warning: 2 };
