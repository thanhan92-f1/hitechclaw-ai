...
async function postReport<T = unknown>(path: string, body: unknown): Promise<T> {
...
The tool returns a download URL the user can click.`,
      ...
      handler: async (args: Record<string, unknown>) => {
        type ExcelResp = { ok: boolean; fileId: string; fileName: string; downloadUrl: string; rows: number; cols: number };
        const resp = await postReport<ExcelResp>('/excel', args);
        return {
          status: 'success',
...
The SVG can be embedded directly in the chat response.`,
      ...
      handler: async (args: Record<string, unknown>) => {
        type ChartResp = { ok: boolean; fileId: string; fileName: string; downloadUrl: string; svgInline: string; type: string };
        const resp = await postReport<ChartResp>('/chart', args);
        return {
          status: 'success',
...
Use this when the user asks for a "full report", "complete analysis", "business report", or mentions both Excel and charts together.`,
      ...
      handler: async (args: Record<string, unknown>) => {
        ...
        const resp = await postReport<ReportResp>('/generate', args);
