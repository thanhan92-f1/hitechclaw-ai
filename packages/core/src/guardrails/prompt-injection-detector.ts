 ...
 */
const SCORE_WEIGHTS = { high: 0.9, medium: 0.5, low: 0.2 } as const;
const BLOCK_THRESHOLD = 0.8;

export class PromptInjectionDetector implements InputGuardrail {
  readonly name = 'prompt-injection-detector';

  async check(input: string, _context: GuardrailContext): Promise<GuardrailResult> {
    const start = Date.now();
    const matched: Array<{ name: string; severity: string }> = [];
    let score = 0;
