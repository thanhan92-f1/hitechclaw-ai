...
export class EvalFramework {
  ...
  private async runTestCase(agent: Agent, testCase: EvalTestCase): Promise<EvalResult> {
  ...
  private async computeMetrics(
    ...
    if (testCase.expectedToolCalls?.length) {
      ...
      const toolMentions = testCase.expectedToolCalls.filter((t) =>
  ...
  private async llmJudge(
    ...
    output: string,
    expected?: string,
  ): Promise<{ relevance: number; hallucination: boolean; usage: TokenUsage }> {
