...
export interface CrawlOptions {
  ...
  sameDomain?: boolean;     // default true — only follow same-domain links
  includePatterns?: string[];  // glob patterns to include (e.g., '/blog/*')
  excludePatterns?: string[];  // glob patterns to exclude (e.g., '/admin/*')
...
export class WebCrawler {
   ...
   */
  async *crawl(startUrl: string): AsyncGenerator<CrawlProgress> {
    this.visited.clear();
    const pages: CrawledPage[] = [];
    const errors: Array<{ url: string; error: string }> = [];
    const queue: Array<{ url: string; depth: number }> = [{ url: this.normalizeUrl(startUrl), depth: 0 }];
    ...
    while (queue.length > 0 && pages.length < this.options.maxPages) {
      ...
      if (!this.shouldCrawl(normalized, startUrl)) continue;
   ...
   */
  private async fetchPage(url: string, depth: number): Promise<CrawledPage> {
  ...
  private shouldCrawl(url: string, startUrl: string): boolean {
    try {
      ...
      // Same-domain check
      if (this.options.sameDomain && target.hostname !== origin.hostname) return false;
      ...
      if (ext && skipExts.includes(ext)) return false;
