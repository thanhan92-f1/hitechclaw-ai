...
export interface RerankerOptions {
  ...
  threshold?: number;     // minimum score to include (default 0.0)
}
...
export class CrossEncoderReranker {
  ...
  private termCoverage(queryTokens: string[], docTokens: string[]): number {
    if (queryTokens.length === 0) return 0;
    const docSet = new Set(docTokens);
    let found = 0;
    for (const qt of queryTokens) {
      if (docSet.has(qt)) found++;
    }
    return found / queryTokens.length;
  }

  private queryTermFrequency(queryTokens: string[], docTokens: string[]): number {
    if (queryTokens.length === 0 || docTokens.length === 0) return 0;
    const querySet = new Set(queryTokens);
    let count = 0;
    for (const dt of docTokens) {
      if (querySet.has(dt)) count++;
    }
    // Normalized TF
    return Math.min(1.0, count / (docTokens.length * 0.1));
  }

  private positionBias(queryTokens: string[], docTokens: string[]): number {
    if (queryTokens.length === 0 || docTokens.length === 0) return 0;
    let totalScore = 0;
    const querySet = new Set(queryTokens);
  ...
  private phraseMatchBonus(query: string, content: string): number {
    ...
    if (words.length >= 3) {
      for (let i = 0; i <= words.length - 3; i++) {
        ...
        if (content.includes(phrase)) return 0.5;
      }
    }
    ...
    if (words.length >= 2) {
      ...
      for (let i = 0; i <= words.length - 2; i++) {
        ...
        if (content.includes(phrase)) found++;
      }
