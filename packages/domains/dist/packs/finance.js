            ...
            tools: [
                {
                    ...
                    execute: async (params) => {
                        const ratios = {};
                        if (params.revenue > 0)
                            ratios.profitMargin = `${((params.netIncome / params.revenue) * 100).toFixed(2)}%`;
                        if (params.totalAssets)
                            ratios.roa = `${((params.netIncome / params.totalAssets) * 100).toFixed(2)}%`;
                        if (params.currentLiabilities)
                            ratios.currentRatio = (params.currentAssets / params.currentLiabilities).toFixed(2);
                        if (params.totalAssets && params.totalLiabilities)
                            ratios.debtToAsset = `${((params.totalLiabilities / params.totalAssets) * 100).toFixed(2)}%`;
