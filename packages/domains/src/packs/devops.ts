...
const JENKINS_TEMPLATES: Record<string, (steps: string[]) => string> = {
...
const DOCKERFILE_TEMPLATES: Record<string, (fw: string, multiStage: boolean) => string> = {
...
function generateDockerCompose(services: string[]): string {
  ...
  for (const svc of services) {
    ...
    if (svcMap[key]) {
      ...
      if (key === 'postgres') volumes.push('  pgdata:');
      if (key === 'mongodb') volumes.push('  mongodata:');
    }
  }
  if (volumes.length) {
      ...
      tools: [
        {
          ...
          execute: async (params: any) => {
            const style = (params.style || 'declarative') as string;
