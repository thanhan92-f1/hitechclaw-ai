...
const DRUG_INTERACTIONS: Array<{ drugs: [string, string]; severity: string; description: string }> = [
...
// Common ICD-10 codes
const ICD10_DB: Array<{ code: string; description: string; keywords: string[] }> = [
      ...
      tools: [
        {
          ...
          execute: async (params: any) => {
            const drugList: string[] = Array.isArray(params.drugs)
              ? params.drugs.map((d: string) => d.toLowerCase().trim())
              : String(params.drugs).split(',').map((d: string) => d.toLowerCase().trim());
          ...
          execute: async (params: any) => {
            ...
            const scored = ICD10_DB.map(entry => {
              ...
              for (const kw of entry.keywords) {
                ...
                else if (kw.split(' ').some(w => query.includes(w))) score += 1;
              }
