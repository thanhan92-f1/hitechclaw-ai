...
export const POLICY_STRICT: SandboxPolicy = {
  name: 'strict',
  version: '1.0.0',
  filesystem: {
    rules: [
...
export const POLICY_DEFAULT: SandboxPolicy = {
  name: 'default',
  version: '1.0.0',
  filesystem: {
    rules: [
...
export const POLICY_PERMISSIVE: SandboxPolicy = {
  name: 'permissive',
  version: '1.0.0',
  filesystem: {
    rules: [
...
export const POLICY_GMAIL: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'gmail',
  network: {
    rules: [
...
export const POLICY_GITHUB: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'github',
  network: {
    rules: [
...
/** Slack integration */
export const POLICY_SLACK: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'slack',
  network: {
    rules: [
...
/** Notion integration */
export const POLICY_NOTION: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'notion',
  network: {
    rules: [
...
export const POLICY_WEB_SEARCH: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'web-search',
  network: {
    rules: [
...
/** Telegram channel */
export const POLICY_TELEGRAM: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'telegram',
  network: {
    rules: [
...
/** Discord channel */
export const POLICY_DISCORD: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'discord',
  network: {
    rules: [
...
/** Zalo channel */
export const POLICY_ZALO: SandboxPolicy = {
  ...POLICY_DEFAULT,
  name: 'zalo',
  network: {
    rules: [
...
export const BUILTIN_POLICIES: Record<string, SandboxPolicy> = {
...
export function registerBuiltinPolicy(name: string, policy: SandboxPolicy): void {
  BUILTIN_POLICIES[name] = policy;
}

/** Map integration IDs to their policies */
export const INTEGRATION_POLICIES: Record<string, SandboxPolicy> = {
  gmail: POLICY_GMAIL,
...
export class PolicyBuilder {
  ...
  constructor(baseName: string = 'custom') {
    this.policy = {
      name: baseName,
      version: '1.0.0',
  ...
  static from(templateName: string): PolicyBuilder {
    ...
    if (!template) throw new Error(`Unknown policy template: ${templateName}`);
  ...
  allowPath(path: string, access: FilesystemPolicyRule['access'] = 'read'): this {
  ...
  inference(provider: string, model: string, stripCredentials: boolean = true): this {
 ...
 */
export function loadPolicyFromYAML(yaml: string): SandboxPolicy {
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith('name:')) {
      ...
      policy.filesystem.defaultAccess = line.split(':')[1]?.trim() as 'none' | 'read';
    } else if (line.startsWith('default:') && section === 'network') {
      policy.network.defaultAction = line.split(':')[1]?.trim() as 'allow' | 'deny';
    } else if (line === 'rules:') {
      subsection = 'rules';
    }
  }
