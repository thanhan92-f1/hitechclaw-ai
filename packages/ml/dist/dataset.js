 ...
 */
function inferValue(val) {
    if (val === '' || val.toLowerCase() === 'null' || val.toLowerCase() === 'na' || val.toLowerCase() === 'nan')
        return null;
...
export function profileDataset(dataset) {
    const columnSchemas = dataset.columns.map((colName, colIdx) => {
...
function inferColumnType(values) {
    ...
    if (uniqueRatio > 0.9 && total > 20)
        return 'id';
    if (uniqueRatio < 0.5 || total <= 20)
        return 'categorical';
    return 'text';
}
...
function estimateMemory(dataset) {
 ...
 */
export function splitDataset(dataset, ratios = [0.8, 0.2], options) {
 ...
 */
export function applyFeaturePipeline(dataset, pipeline) {
...
function applyTransform(rows, columns, colIdx, transform) {
    const newColumns = [...columns];
    let newRows = rows.map((r) => [...r]);
    switch (transform.type) {
        case 'normalize': {
            const nums = newRows.map((r) => r[colIdx]).filter((v) => typeof v === 'number');
        ...
        case 'impute': {
            ...
            if (transform.method === 'mean') {
                ...
                const nums = nonNull.filter((v) => typeof v === 'number').sort((a, b) => a - b);
