...
import { profileDataset, splitDataset } from './dataset.js';
...
export class AutoMLEngine {
    ...
    async runPipeline(dataset, config, options) {
        const pipeline = {
        ...
        try {
            ...
            // 3. Split data
            const split = splitDataset({ ...dataset, rows: xData.map((x, i) => [...x, yData[i]]) }, [config.trainTestSplit, 1 - config.trainTestSplit]);
            const nFeatures = featureNames.length;
            const xTrain = split.train.rows.map((r) => r.slice(0, nFeatures));
            const yTrain = split.train.rows.map((r) => r[nFeatures]);
            const xTest = split.test.rows.map((r) => r.slice(0, nFeatures));
            ...
            for (const algorithm of candidateAlgorithms) {
                if (trialNumber >= config.maxTrials)
                    break;
                ...
                const hpCombinations = generateHyperparameters(algorithm.hyperparameters, config.strategy, Math.min(config.maxTrials - trialNumber, 3));
                for (const hpCombo of hpCombinations) {
                    ...
                    try {
                        const result = trainModel(algorithm.id, xTrain, yTrain, xTest, yTest, hpCombo, featureNames);
                        const trial = {
                            trialNumber,
                        ...
                        pipeline.logs.push(`Trial ${trialNumber}: ${algorithm.name} — ${formatMetric(config.optimizeMetric, result.metrics)}`);
                        // Track best
            ...
            if (pipeline.bestTrialIndex !== undefined) {
                ...
                pipeline.logs.push(`Best model: ${model.name} (${formatMetric(config.optimizeMetric, bestTrial.metrics)})`);
            }
    ...
    /** List all pipelines */
    listPipelines() {
        return [...this.pipelines.values()].sort((a, b) => (b.startedAt?.getTime() ?? 0) - (a.startedAt?.getTime() ?? 0));
    }
...
// ─── Helpers ────────────────────────────────────────────────
function prepareData(dataset, targetIdx) {
...
function generateHyperparameters(defs, strategy, count) {
    const results = [];
...
function formatMetric(metricName, metrics) {
    const m = metrics;
...
function isBetter(a, b, metric) {
    ...
    if (['mse', 'rmse', 'mae', 'mape'].includes(metric))
        return aVal < bVal;
