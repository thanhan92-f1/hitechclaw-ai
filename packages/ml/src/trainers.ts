...
export function trainModel(
  ...
  switch (algorithmId) {
    case 'linear-regression':
      return trainLinearRegression(xTrain, yTrain as number[], xTest, yTest as number[], hyperparams);
...
function trainLinearRegression(
  ...
  xTest: number[][], yTest: number[],
  hp: Hyperparameters,
): TrainerResult {
  const lr = (hp.learningRate as number) ?? 0.01;
  const epochs = (hp.epochs as number) ?? 100;
  ...
  for (let epoch = 0; epoch < epochs; epoch++) {
    ...
    for (let i = 0; i < m; i++) {
      let pred = bias;
      for (let j = 0; j < nFeatures; j++) pred += weights[j] * xTrain[i][j];
      const error = pred - yTrain[i];
      gradB += error;
      for (let j = 0; j < nFeatures; j++) gradW[j] += error * xTrain[i][j];
    }

    bias -= (lr * gradB) / m;
    for (let j = 0; j < nFeatures; j++) weights[j] -= (lr * gradW[j]) / m;
  }

  const predict = (input: number[][]): number[] =>
    input.map((row) => {
      let pred = bias;
      for (let j = 0; j < nFeatures; j++) pred += weights[j] * row[j];
      return pred;
    });
...
function trainLogisticRegression(
  xTrain: number[][], yTrain: unknown[],
  xTest: number[][], yTest: unknown[],
  hp: Hyperparameters,
): TrainerResult {
  const lr = (hp.learningRate as number) ?? 0.01;
  const epochs = (hp.epochs as number) ?? 100;
  ...
  if (classes.length <= 2) {
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
      ...
      for (let i = 0; i < m; i++) {
        let z = bias;
        for (let j = 0; j < nFeatures; j++) z += weights[j] * xTrain[i][j];
    ...
    const predict = (input: number[][]): unknown[] =>
      input.map((row) => {
        let z = bias;
  ...
  const predict = (input: number[][]): unknown[] =>
    input.map((row) => {
      ...
      for (let c = 0; c < classes.length; c++) {
        let z = allBiases[c];
        for (let j = 0; j < nFeatures; j++) z += allWeights[c][j] * row[j];
        if (z > bestScore) {
          bestScore = z;
          bestClass = c;
        }
...
function trainDecisionTree(
  ...
  const maxDepth = (hp.maxDepth as number) ?? 10;
  const minSamplesSplit = (hp.minSamplesSplit as number) ?? 2;
  ...
  const predict = (input: number[][]): unknown[] =>
    input.map((row) => predictTree(tree, row));
  ...
  if (isRegression) {
    metrics = computeRegressionMetrics(predictions as number[], yTest as number[]);
  } else {
    const classes = [...new Set(yTrain.map(String))].sort();
    const labelMap = new Map(classes.map((c, i) => [c, i]));
...
function buildTree(
  X: number[][], y: unknown[], depth: number,
  maxDepth: number, minSamplesSplit: number, isRegression: boolean,
): TreeNode {
  if (depth >= maxDepth || X.length < minSamplesSplit || allSame(y)) {
    return { value: isRegression ? mean(y as number[]) : mode(y) };
  }
  ...
  for (let f = 0; f < nFeatures; f++) {
    ...
    for (let t = 0; t < values.length - 1; t++) {
      ...
      const impurity = isRegression
        ? (leftIdx.length * variance(yLeft as number[]) + rightIdx.length * variance(yRight as number[])) / X.length
...
function predictTree(node: TreeNode, row: number[]): unknown {
  if (node.value !== undefined) return node.value;
  if (row[node.featureIdx!] <= node.threshold!) return predictTree(node.left!, row);
  return predictTree(node.right!, row);
}

function countSplits(node: TreeNode, map: Map<number, number>): void {
  if (node.featureIdx !== undefined) {
    map.set(node.featureIdx, (map.get(node.featureIdx) ?? 0) + 1);
    if (node.left) countSplits(node.left, map);
    if (node.right) countSplits(node.right, map);
  }
}
...
function trainKNN(
  ...
  const predict = (input: number[][]): unknown[] =>
    input.map((row) => {
      ...
      if (isRegression) {
        return mean(neighbors.map((n) => n.label as number));
      }
...
function trainKMeans(
  xTrain: number[][], xTest: number[][],
  hp: Hyperparameters,
): TrainerResult {
  const nClusters = (hp.nClusters as number) ?? 3;
  const maxIter = (hp.maxIter as number) ?? 300;
  const nFeatures = xTrain[0].length;

  // Initialize centroids (kmeans++)
  const centroids: number[][] = [xTrain[Math.floor(Math.random() * xTrain.length)]];
  for (let c = 1; c < nClusters; c++) {
    ...
    if (centroids.length === c) centroids.push([...xTrain[Math.floor(Math.random() * xTrain.length)]]);
  }
...
function trainNaiveBayes(
  ...
  const alpha = (hp.alpha as number) ?? 1.0;
  const classes = [...new Set(yTrain.map(String))].sort();
  const nFeatures = xTrain[0].length;
  ...
  const classStds = new Map<string, number[]>();
...
function trainRandomForest(
  ...
  const nEstimators = Math.min((hp.nEstimators as number) ?? 20, 50); // Cap for perf
  const maxDepth = (hp.maxDepth as number) ?? 8;
  const isRegression = yTrain.every((y) => typeof y === 'number');
  ...
  const importanceMap = new Map<number, number>();
  for (const tree of trees) countSplits(tree, importanceMap);
  const totalSplits = [...importanceMap.values()].reduce((s, v) => s + v, 0) || 1;
  const featureImportance = [...importanceMap.entries()]
    .map(([idx, count]) => ({ feature: featureNames[idx] ?? `feature_${idx}`, importance: count / totalSplits }))
    .sort((a, b) => b.importance - a.importance);
...
function trainGradientBoosting(
  ...
  const nEstimators = Math.min((hp.nEstimators as number) ?? 50, 100);
  const learningRate = (hp.learningRate as number) ?? 0.1;
  const maxDepth = (hp.maxDepth as number) ?? 4;
  ...
  if (isClassification) {
    ...
    const classEnsembles: { basePred: number; trees: TreeNode[] }[] = [];
    ...
    for (let c = 0; c < classes.length; c++) {
      ...
      for (let t = 0; t < nEstimators; t++) {
        ...
        residuals = residuals.map((_, i) => {
          const pred = predictTree(tree, xTrain[i]) as number;
      ...
      input.map((row) => {
        ...
        for (let c = 0; c < classes.length; c++) {
          ...
          for (const tree of classEnsembles[c].trees) {
            score += learningRate * (predictTree(tree, row) as number);
          }
          if (score > bestScore) {
            bestScore = score;
            bestClass = c;
    ...
    for (const ens of classEnsembles) {
      for (const tree of ens.trees) countSplits(tree, importanceMap);
    }
    const totalSplits = [...importanceMap.values()].reduce((s, v) => s + v, 0) || 1;
    const featureImportance = [...importanceMap.entries()]
      .map(([idx, count]) => ({ feature: featureNames[idx] ?? `feature_${idx}`, importance: count / totalSplits }))
  ...
  // Regression path
  const yTrainNum = yTrain as number[];
  const yTestNum = yTest as number[];
  ...
  const predict = (input: number[][]): number[] =>
    input.map((row) => {
      ...
      for (const tree of trees) {
        pred += learningRate * (predictTree(tree, row) as number);
      }
      return pred;
    });
...
function computeRegressionMetrics(predictions: number[], actual: number[]): RegressionMetrics {
...
function computeClassificationMetrics(
  predictions: number[], actual: number[], nClasses: number,
): ClassificationMetrics {
  const n = predictions.length;
  let correct = 0;
...
function giniImpurity(values: unknown[]): number {
  const freq = new Map<string, number>();
  for (const v of values) {
    const key = String(v);
    freq.set(key, (freq.get(key) ?? 0) + 1);
