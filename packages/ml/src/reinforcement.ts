...
export class BanditSelector {
  ...
  select(availableArms?: string[]): string {
    ...
    if (armIds.length === 0) throw new Error('No arms available for selection');
    if (armIds.length === 1) return armIds[0];

    // Ensure all arms are registered
    for (const id of armIds) this.addArm(id);
  ...
  getRanking(): Array<{ armId: string; stats: ArmStats }> {
  ...
  private selectEpsilonGreedy(armIds: string[]): string {
  ...
  private selectUCB1(armIds: string[]): string {
  ...
  private selectThompsonSampling(armIds: string[]): string {
    let bestArm = armIds[0];
  ...
  private bestMeanArm(armIds: string[]): string {
    let bestArm = armIds[0];
