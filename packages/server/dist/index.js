...
import { Agent, AnthropicAdapter, ApprovalManager, DeepSeekAdapter, EvalFramework, GeminiAdapter, GroqAdapter, HuggingFaceAdapter, ImageGenService, isCoordinatorModeEnabled, LangGraphWorkflowEngine, LocalEmbeddingProvider, MonitoringService, MultiAgentOrchestrator, OllamaAdapter, OpenAIAdapter, OpenAIEmbeddingProvider, PluginManager, RagEngine, } from '@hitechclaw/core';
...
// Load env
const { PORT = '5001', HOST = '0.0.0.0', CORS_ORIGINS = 'http://localhost:3000,http://localhost:3001,http://localhost:3002', JWT_SECRET = 'hitechclaw-dev-secret-change-me', LLM_PROVIDER = 'ollama', LLM_MODEL: LLM_MODEL_ENV, OPENAI_API_KEY = '', ANTHROPIC_API_KEY = '', OLLAMA_BASE_URL = 'http://localhost:11434/v1', HUGGINGFACE_API_KEY = '', DEEPSEEK_API_KEY = '', GROQ_API_KEY = '', AGENT_NAME = 'HiTechClaw Assistant', AGENT_SYSTEM_PROMPT = '', IMAGE_GEN_PROVIDER = 'placeholder', GEMINI_API_KEY = '', REPLICATE_API_KEY = '', TOGETHER_API_KEY = '', COMFYUI_URL = 'http://localhost:8188', OPENSHELL_ENABLED = '', OPENSHELL_GATEWAY_URL = '', 
...
VISION_PROVIDER = 'ollama', VISION_MODEL = '', 
// Channel tokens — auto-seeded to MongoDB on first startup (idempotent, per default tenant)
TELEGRAM_BOT_TOKEN = '', DISCORD_BOT_TOKEN = '', SLACK_BOT_TOKEN = '', SLACK_SIGNING_SECRET = '', WHATSAPP_PHONE_NUMBER_ID = '', WHATSAPP_ACCESS_TOKEN = '', WHATSAPP_VERIFY_TOKEN = '', FACEBOOK_PAGE_ACCESS_TOKEN = '', FACEBOOK_VERIFY_TOKEN = '', } = process.env;
...
function wantsRealtimeSearch(message) {
...
function buildRoutingInstruction(opts) {
    const instructions = [];
    if (opts.hasImages) {
        instructions.push('User attached image(s). Prioritize visual understanding from attached images before answering. If text in image is important, extract it first, then answer based on both image and user text.');
    }
    if (opts.wantsWebSearch) {
        instructions.push('This query likely needs up-to-date information. If a web-search tool is available, use it first and cite sources. If no web-search tool is available, clearly state that the answer may be outdated.');
    }
...
async function main() {
    ...
    try {
        ...
        console.warn('⚠️  Seed skipped (DB may not be ready):', err.message);
    }
    ...
    try {
        ...
        console.warn('⚠️  Channel env-seed skipped:', err.message);
    }
    ...
    try {
        ...
        console.warn('⚠️  Agent config seed skipped:', err.message);
    }
    // Agent config
    const agentConfig = {
        id: 'default-agent',
    ...
    if (OPENAI_API_KEY && LLM_PROVIDER !== 'ollama') {
        agentManager.registerAdapter(new OpenAIAdapter({ apiKey: OPENAI_API_KEY, model: LLM_MODEL }));
    }
    if (ANTHROPIC_API_KEY) {
        agentManager.registerAdapter(new AnthropicAdapter({
            apiKey: ANTHROPIC_API_KEY,
            model: LLM_PROVIDER === 'anthropic' ? LLM_MODEL : 'claude-sonnet-4-20250514',
        }));
    }
    if (HUGGINGFACE_API_KEY) {
        agentManager.registerAdapter(new HuggingFaceAdapter({
            apiKey: HUGGINGFACE_API_KEY,
            model: LLM_PROVIDER === 'huggingface' ? LLM_MODEL : 'meta-llama/Llama-3.1-70B-Instruct',
        }));
    }
    if (DEEPSEEK_API_KEY) {
        agentManager.registerAdapter(new DeepSeekAdapter({
            apiKey: DEEPSEEK_API_KEY,
            model: LLM_PROVIDER === 'deepseek' ? LLM_MODEL : 'deepseek-chat',
        }));
    }
    if (GROQ_API_KEY) {
        agentManager.registerAdapter(new GroqAdapter({
            apiKey: GROQ_API_KEY,
            model: LLM_PROVIDER === 'groq' ? LLM_MODEL : 'llama-3.3-70b-versatile',
        }));
    }
    if (GEMINI_API_KEY) {
        agentManager.registerAdapter(new GeminiAdapter({
            apiKey: GEMINI_API_KEY,
            model: LLM_PROVIDER === 'google' ? LLM_MODEL : 'gemini-2.0-flash',
        }));
    }
    ...
    monitoring.setStore(mongoMonitoringStore);
    ...
    const makeChannelHandler = (platform, send) => async (incoming) => {
        ...
        if (trimmed === '/debug' || trimmed === '/debug on' || trimmed === '/debug off' || trimmed.startsWith('/debug ')) {
            try {
                ...
                const caps = agentCfg.llm.capabilities || {};
        ...
        try {
            ...
            if (debugSessions.has(debugKey)) {
                const caps = channelAgent.config.llm.capabilities || {};
                finalResponse += `\n\n━━━ 🔍 DEBUG ━━━\n`
    ...
    if (OPENSHELL_ENABLED === 'true') {
        try {
            ...
            console.warn('⚠️  OpenShell sandbox skipped:', err.message);
    ...
    // Start server
    serve({ fetch: app.fetch, hostname: gatewayConfig.host, port: gatewayConfig.port }, (info) => {
