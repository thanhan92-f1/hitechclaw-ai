...
export function docsList(category?: string): void {
...
export function docsSearch(query: string): void {
    ...
    const results: { path: string; score: number; title: string }[] = [];
...
/** Add a new document */
export function docsAdd(relPath: string, title: string, tags: string): void {
    const docsRoot = getDocsRoot();
