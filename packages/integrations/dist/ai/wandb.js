...
async function wandbRequest(method, path, apiKey, body, params) {
    ...
    actions: [
        {
            ...
            execute: async (args, ctx) => {
                ...
                if (!apiKey)
                    return { success: false, error: 'W&B API key not configured' };
                if (!entity)
                    return { success: false, error: 'W&B entity not configured' };
                try {
                    // Create run then update its summary with metrics
                    const runBody = { project: args.project };
                    if (args.runName)
                        runBody.display_name = args.runName;
                    if (args.config)
                        runBody.config = args.config;
                    if (args.tags)
