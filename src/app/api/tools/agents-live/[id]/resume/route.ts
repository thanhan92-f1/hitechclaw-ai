...
import { resumeRun } from "@/lib/active-runs";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (!result.rows[0]) {
      return NextResponse.json({ error: "Run not found or not paused" }, { status: 404 });
    }

    const row = result.rows[0] as Record<string, unknown>;
    const activeRun = resumeRun(id);
    ...
    console.error("[tools/agents-live/:id/resume] POST error:", error);
